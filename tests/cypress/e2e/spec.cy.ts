...
describe('Friday App API', () => {
  ...
  it('returns 200 for isFriday', () => {
    cy.request('GET', `https://p6n3heom5d.execute-api.eu-central-1.amazonaws.com/isFriday/1674754910507`).then((response) => {
      expect(response.status).eq(200);
    })
  })

  it('returns 200 for minutesToFriday', () => {
    cy.request('GET', `https://p6n3heom5d.execute-api.eu-central-1.amazonaws.com/minutesToFriday/1674754910507`).then((response) => {
      expect(response.status).eq(200);
    })
  })
